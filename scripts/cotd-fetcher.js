...
async function fetchRetry(url,opts={},retries=5,baseDelay=500){let lastErr;for(let i=0;i<=retries;i++){try{const r=await fetch(url,opts);if(r.status===429||(r.status>=500&&r.status<=599)){const wait=Math.min(baseDelay*Math.pow(2,i),8000);await sleep(wait);continue;}return r;}catch(e){lastErr=e;const wait=Math.min(baseDelay*Math.pow(2,i),8000);await sleep(wait);}}throw lastErr||new Error(`fetch failed for ${url}`);}

if(!NADEO_REFRESH_TOKEN){console.error("ERROR: Missing NADEO_REFRESH_TOKEN env.");process.exit(1);}
const tokenCache=new Map();
async function refreshForAudience(audience){const r=await fetch(`${CORE}/v2/authentication/token/refresh`,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`nadeo_v1 t=${NADEO_REFRESH_TOKEN}`},body:JSON.stringify({audience})});if(!r.ok){const txt=await r.text().catch(()=>"");throw new Error(`refresh(${audience}) failed: ${r.status} ${txt}`);}const j=await r.json();const token=j.accessToken;const expMs=(j.expiresIn?j.expiresIn*1000:9*60*1000);tokenCache.set(audience,{token,expAt:Date.now()+expMs});return token;}
async function getToken(audience){const entry=tokenCache.get(audience);if(entry&&Date.now()<entry.expAt-10000)return entry.token;return refreshForAudience(audience);}
async function authedFetch(url,{audience,init={},retryOnAuth=true}={}){let token=await getToken(audience);let r=await fetch(url,{...init,headers:{...(init.headers||{}),Authorization:`nadeo_v1 t=${token}`}});if(retryOnAuth&&(r.status===401||r.status===403)){token=await refreshForAudience(audience);r=await fetch(url,{...init,headers:{...(init.headers||{}),Authorization:`nadeo_v1 t=${token}`}});}return r;}
...
async function rebuildMonthIndex(dir){await ensureDir(dir);const items=await readdir(dir,{withFileTypes:true});const months=items.filter(e=>e.isFile()&&e.name.endsWith(".json")&&e.name!=="months.json"&&!e.name.startsWith("_")).map(e=>e.name.replace(/\.json$/,"")).sort().reverse();await writeJson(path.join(dir,"months.json"),{months});}

function stripTmFormatting(input){if(!input||typeof input!=="string")return input;const D="\uFFF0";let s=input.replace(/\$\$/g,D);s=s.replace(/\$[0-9a-fA-F]{1,3}|\$[a-zA-Z]|\$[<>\[\]\(\)]/g,"");return s.replace(new RegExp(D,"g"),"$");}
function toMs(dt){if(dt==null)return NaN;let n=typeof dt==="string"?Number(dt):dt;if(Number.isFinite(n)){if(n<2e12)n*=1000;return n;}const p=Date.parse(dt);return Number.isFinite(p)?p:NaN;}

function extractWinnerFields(result){const p=result?.participant??result?.player??null;let accountId=null,displayName=null;if(typeof p==="string")accountId=p;if(!accountId&&p&&typeof p==="object"){accountId=p.accountId||p.id||p.player?.accountId||p.player?.id||null;displayName=p.displayName||p.name||p.player?.displayName||p.player?.name||result?.displayName||result?.name||null}if(!displayName)displayName=result?.playerName||result?.nickname||null;return{accountId:accountId||null,displayName:displayName||null};}

async function fetchTmioMonth(index=0){const r=await fetchRetry(`${TMIO}/api/totd/${index}`,{headers:{"User-Agent":"tm-cotd"}});if(!r.ok)throw new Error(`tm.io totd[${index}] failed: ${r.status}`);return r.json();}
function tmioMonthYear(resp){const y=resp?.month?.year??new Date().getUTCFullYear();const m1=(resp?.month?.month??new Date().getUTCMonth())+1;return{y,m1};}
function tmioDayNumber(dayObj,idx){return dayObj?.day??dayObj?.dayIndex??dayObj?.monthDay??dayObj?.dayInMonth??(idx+1);}
function normTmioEntry(y,m1,entry,idx){const m=entry.map||entry;const uid=m.mapUid??entry.mapUid??null;let name=m.name??m.mapName??entry.name??"(unknown map)";let authorDisplayName=m.authorPlayer?.name??m.authorplayer?.name??m.authorName??m.author??entry.authorPlayer?.name??entry.authorplayer?.name??"(unknown)";const thumb=m.thumbnail??m.thumbnailUrl??entry.thumbnail??entry.thumbnailUrl??"";const authorAccountId=m.authorPlayer?.accountId??m.authorplayer?.accountid??entry.authorPlayer?.accountId??entry.authorplayer?.accountid??null;const d=tmioDayNumber(entry,idx);name=stripTmFormatting(name);authorDisplayName=stripTmFormatting(authorDisplayName);return{date:dateKey(y,m1,d),map:{uid,name,authorAccountId,authorDisplayName,thumbnailUrl:thumb}};}
async function writeTotdMonth(index=0){const j=await fetchTmioMonth(index);const {y,m1}=tmioMonthYear(j);const mKey=monthKey(y,m1);const daysOut={};(Array.isArray(j.days)?j.days:[]).forEach((entry,i)=>{const rec=normTmioEntry(y,m1,entry,i);daysOut[rec.date]=rec;});await ensureDir(TOTD_DIR);await writeJson(path.join(TOTD_DIR,`${mKey}.json`),{month:mKey,days:daysOut});await rebuildMonthIndex(TOTD_DIR);const keys=Object.keys(daysOut).sort();const latestKey=keys[keys.length-1]||null;if(latestKey){const latestTotd=daysOut[latestKey];await writeJson(TOTD_LATEST,{generatedAt:new Date().toISOString(),...latestTotd});dlog("TOTD latest:",latestTotd.date,latestTotd.map?.name);}else{dlog("TOTD: no days found for",mKey);}return{mKey,daysOut};}

function looksLikeCotd(c){const name=String(c?.name||"").toLowerCase();return name.includes("cup of the day")||name.includes("cotd")||/\bday\b/.test(name)||/#\d{1,3}/.test(name)||name.includes("cup du jour");}

async function listCompetitions(offset=0,length=100){const r=await fetchMeet(`${MEET}/api/competitions?offset=${offset}&length=${length}`);if(!r.ok)throw new Error(`competitions list failed: ${r.status}`);return r.json();}
...
async function findCotdCompetitionByDate(y,m1,d){
  ...
  for(let page=0;page<MAX_PAGES;page++){
    const offset=page*PAGE_LEN,data=await listCompetitions(offset,PAGE_LEN);
  ...
  if(!pick){pick=hits.reduce((best,cur)=>{const bt=toMs(best?.startDate??best?.beginDate??best?.startTime??best?.beginTime);const ct=toMs(cur?.startDate??cur?.beginDate??cur?.startTime??cur?.beginTime);return ct<bt?cur:best;},hits[0]);}
...
async function getD1WinnerForCompetition(compId){
  ...
  const finalRound=rounds.find(r=>String(r?.name??"").toLowerCase().includes("final"))??rounds.reduce((a,b)=>((a?.position??0)>(b?.position??0)?a:b));
  ...
  for(let offset=0;offset<2000;offset+=PAGE){
    ...
    const j=await res.json();const batch=j?.matches||j||[];if(!batch.length)break;allMatches.push(...batch);if(batch.length<PAGE)break;
  }
  ...
  if(!d1){d1=allMatches.reduce((best,cur)=>{const bp=best?.position??best?.number??Number.POSITIVE_INFINITY;const cp=cur?.position??cur?.number??Number.POSITIVE_INFINITY;return cp<bp?cur:best;},allMatches[0]);}
  ...
  arr.sort((a,b)=>{const ar=a.rank??a.position??Infinity,br=b.rank??b.position??Infinity;if(ar!==br)return ar-br;const ap=typeof a.points==="number"?-a.points:0,bp=typeof b.points==="number"?-b.points:0;return ap-bp;});
  const top=arr[0];const {accountId,displayName}=extractWinnerFields(top);
...
async function fetchDisplayNamesBulk(ids){
  ...
  const uniq=[...new Set(ids)].filter(Boolean);const chunks=[];
  for(let i=0;i<uniq.length;i+=100)chunks.push(uniq.slice(i,i+100));
  const out={};
  for(const chunk of chunks){
    const url=new URL(`${CORE}/accounts/displayNames`);url.searchParams.set("accountIdList",chunk.join(","));
    const r=await fetchCore(url.toString());if(!r.ok)throw new Error(`displayNames failed: ${r.status}`);
    const arr=await r.json();for(const it of arr||[]){if(it?.accountId)out[it.accountId]=it.displayName||null;}
  }
  return out;
}
async function hydrateDisplayNames(ids){
  const cache=await loadNamesCache();const missing=ids.filter(id=>cache[id]===undefined);
  if(missing.length){const fetched=await fetchDisplayNamesBulk(missing);for(const id of missing)cache[id]=fetched[id]??null;await saveNamesCache(cache);}
  const map={};for(const id of ids)map[id]=cache[id]??null;return map;
}
...
async function updateCotdCurrentMonth(){
  ...
  for(const d of daysOfMonth(y,m1)){
    ...
    }catch(e){console.log(`[COTD] ${dk} error: ${e.message}`);}
  }
...
async function main(){
  await ensureDir(TOTD_DIR);await ensureDir(COTD_DIR);
