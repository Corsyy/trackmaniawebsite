...
async function getJson(url) {
  const r = await fetch(url, { headers: { "User-Agent": "trackmaniaevents.com (cotd-fetcher)" } });
...
async function enrichByMapUid(uid) {
  if (!uid) return {};
  try {
    // trackmania.io map lookup (public). If this changes, enrichment just silently fails.
